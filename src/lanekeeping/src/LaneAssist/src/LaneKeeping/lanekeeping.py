...
class LaneKeeping:
    ...
    def change_lane_maneuver(self, lanes_detection, direction, overtake_type, smoothing_iterations=None):
        ...
        cv2.putText(
            frame,
            f"NDRTS ~ Changing to {direction} lane...",
    ...
    def change_lane(self, lanes_detection, destination):
        ...
        if not self.turn_left and not self.turn_right:
            if self.change_lane_stage_1:
                ...
                self.log.debug(
                    "NDRTS ~ Info: Lane change interrupted - remaining in current lane due to conflicting turn signal.\n"
                )
                return self.angle, frame
            else:
                self.reset_change_lane_params()
        ...
        if self.change_lane_stage_1:
            if (self.turn_left and not left_lane) or (self.turn_right and not right_lane):
                ...
                self.angle, frame = self.lane_keeping(lanes_detection)
                self.log.debug(
                    "\n\n\n\nNDRTS ~ Error: Unable to change lanes. - Correct lane did not detected.\n\n\n\n"
                )
                return self.angle, frame
            if self.turn_left:
            ...
            if self.count >= count:
                self.reset_change_lane_params()
                self.log.debug("NDRTS ~ Info: Lane change completed successfully.\n")
            else:
                self.count += 1
        self.fix_angle()
    ...
    def pid_controller(self, left, right, frame=None):
        ...
        if left is None and right is None:
            ...
            error = 0
            self.log.warning("NDRTS ~ Error: No lanes found")
        else:
    ...
    def lane_keeping(self, lanes_detection):
        ...
        send_data(self.angle)
        #print('NDRTS ~ Info: Steering comamand sent:',self.angle)
        return self.angle, frame
